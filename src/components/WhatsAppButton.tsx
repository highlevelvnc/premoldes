...
export default function WhatsAppButton() {
  ...
  return (
    <a
      ...
      className={`fixed bottom-5 right-5 z-50 flex items-center gap-3 transition-all duration-500 ease-smooth ${
        show ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-6 opacity-0"
      }`}
    >
      <span
        className={`hidden rounded-full bg-ink-900 px-4 py-2 text-sm font-medium text-white shadow-cardlg transition-all duration-300 sm:block ${
          tip ? "translate-x-0 opacity-100" : "pointer-events-none translate-x-2 opacity-0"
        }`}
      >
        Peça o seu orçamento 👋
      </span>
      <span className="relative flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] ring-4 ring-white shadow-cardlg transition-transform duration-300 hover:scale-105">
